        ...
        <div class="chat-footer">
           <div class="chat-input-area">
                ...
                <form id="chat-form" class="chat-form">
                    <button type="button" id="attach-button" class="icon-button" aria-label="Anexar arquivo">${ICONS.paperClip}</button>
                    <input type="file" id="file-input" accept="image/*" class="hidden" />
                    <textarea id="message-input" rows="1" placeholder="Message" class="message-input"></textarea>
                    <button type="submit" id="send-button" class="send-button" disabled>${ICONS.send}</button>
                </form>
           </div>
        </div>
    </div>`;
...
function render() {
    appContent.innerHTML = renderChatScreen();
    bindDOM();
    renderMessages();
    updateChatUI();
}
